"""Product Hunt scraper — parses __NEXT_DATA__ JSON.
...
class ProductHuntScraper(BaseScraper):
    ...
    def scrape(self) -> Iterator[FeedbackItem]:
        ...
        logger.info("[product_hunt] Scraping %s", slug)
        ...
        while yielded < self.max_items:
            ...
            try:
                ...
                if resp.status_code == 429:
                    logger.warning("[product_hunt] 429 — stopping")
                    break
                resp.raise_for_status()
            except Exception as exc:
                logger.error("[product_hunt] Request failed (page %d): %s", page, exc)
                break
                ...
                try:
                    ...
                    for r in reviews:
                        ...
                        try:
                            ...
                            logger.warning("[product_hunt] Skipping review: %s", exc)
                except Exception as exc:
            ...
            if items_found == 0:
                logger.info("[product_hunt] No items on page %d — stopping", page)
                break
